...
6. **升级 · 向用户**：worker 撞上死结用 `escalate`（`blocking=false` 问而不停 / `blocking=true` 挂起求决策）经 CEO 中转到用户；CEO 自己在关键岔路 / 开工提案用 `ask_user`。→ [协作模式 §二·escalate](/docs/03-AI核心/Agent协作模式.md) · [检查点与开工卡 · §四](/docs/03-AI核心/检查点与开工卡.md)
...
| 原语 · `ask_user` | CEO 唯一发问 | 开工提案卡 + 途中拍板；`blocking` 正交维度 | [检查点与开工卡 · §四](/docs/03-AI核心/检查点与开工卡.md) |
