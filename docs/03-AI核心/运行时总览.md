...
| 恋综场景（第二场景） | `simulation/scenarios/show/` + `simulation/show/` | 6 人卡司、`heart_pick` 赛制状态机、期编排器、导播编译器（run → EpisodeManifest）、节目 API；离线生产：`python -m agentcore.simulation.show produce --episode 3`；产品定案 → [AgentTown 客户端](/docs/04-前端/AgentTown客户端.md)（恋综提案全文：详细提案不在公开仓 / 维护者本地） |
