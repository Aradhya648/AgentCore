...
设置拆「模型」（组合）/「服务商」（凭据）；本地引擎挂外观。会话选**组合**非裸模型。**否决**角色→模型矩阵、质量档、双 picker。自主度配方四选一（谨慎 / 写代码 / 少打断 / 托管）→ [安全权限与治理 §三](/docs/05-平台与运维/安全权限与治理.md)。账户默认：桌面经对话权限徽章「设为新会话默认」写入；手机仍可经设置改。会话内徽章可改三轴；非法组合不可选；自定义三轴不可存为账户默认。
