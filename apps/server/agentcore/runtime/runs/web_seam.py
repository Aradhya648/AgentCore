...
def web_paths_under_scope(scope: str, workspace_paths: Iterable[str]) -> list[str]:
    """Web artifact paths under ``scope`` (directory prefix), stable order."""
    prefix = _normalize_scope_prefix(scope)
    if not prefix:
        return web_paths(workspace_paths)
    out: list[str] = []
    seen: set[str] = set()
    for p in web_paths(workspace_paths):
        norm = _normalize_artifact_relpath(p)
        if not norm:
            continue
        if (norm == prefix.rstrip("/") or norm.startswith(prefix)) and p not in seen:
            seen.add(p)
            out.append(p)
    return out
...
def _normalize_scope_prefix(scope: str) -> str:
    raw = (scope or "").replace("\\", "/").strip().lstrip("./")
    if not raw:
        return ""
...
def _normalize_artifact_relpath(path: str) -> str:
    ...
    if not raw:
        return ""
    stripped = strip_root_label_prefix(raw, "workspace")
    if stripped in (".", ""):
        return ""
    return stripped.lstrip("./")


def _scoped_web_contents(
    scope: str,
    artifact_contents: Mapping[str, str] | None,
) -> dict[str, str]:
    """Filter ``artifact_contents`` to web files under ``scope``."""
    ...
    for path, text in artifact_contents.items():
        if text is None or not path:
            continue
        norm = _normalize_artifact_relpath(path)
        if not norm:
            continue
        if (
            (not prefix or norm == prefix.rstrip("/") or norm.startswith(prefix))
            and _ext(path) in _WEB_EXTS
        ):
            out[path] = text
    return out


def check_integrated_web_seam_failures(
    scope: str,
    artifact_contents: Mapping[str, str] | None,
    *,
    workspace_paths: Iterable[str] | None = None,
) -> list[str]:
    """Integrated site seam check: all HTML/CSS/JS under ``scope``, not one worker batch.

    Logs ``web_seam.final_check`` with pass/fail and file counts for production traces.
    """
    scoped = _scoped_web_contents(scope, artifact_contents)
    paths = web_paths_under_scope(scope, workspace_paths or [])
    failures = check_web_seam_failures(scoped)
    html_n = sum(1 for p in scoped if _ext(p) in _HTML_EXTS)
    css_n = sum(1 for p in scoped if _ext(p) in _STYLE_EXTS)
    js_n = sum(1 for p in scoped if _ext(p) in _SCRIPT_EXTS)
    logger.info(
        "web_seam.final_check",
        scope=scope or None,
        web_paths=len(paths),
        loaded=len(scoped),
        html_files=html_n,
