...
def wind_down_allowed_tools(*, keep_file_read: bool = False) -> frozenset[str]:
    """Effective wind-down whitelist (optionally including file_read for deliverables)."""
    if keep_file_read:
        return WIND_DOWN_ALLOWED_TOOLS | {WIND_DOWN_FILE_READ}
    return WIND_DOWN_ALLOWED_TOOLS
...
def narrow_tools_for_wind_down(
    ...
    *,
    ...
    When ``keep_file_read`` is None, infer from the live tool surface (files deliverable
    heuristic). Retrieval tools (web_search / read_url / …) stay excluded.
    """
