...
（2–3 个气质方向）；演讲/PPT/课件/演示文稿**必须**非空 `format_options`（pptx / marp / outline；\
有执行默认偏 pptx，无执行默认 marp 并明示）；Agent/自动化/工作流/流水线（或明确多步骤代运营）\
**必须**非空 `format_options` 三档（可运行自动化 / 控制台原型 / 仅方案；与演讲共用字段、分流记账）；\
软件/应用须把「交付形态」放进 `questions`（可运行单页 / 多文件小工具 / 前后端 / 仅文档），\
...
或手写同构：N 角调研/讨论笔记 → 提纲 → 撰稿），**【禁止】一人包办「自搜+成文」**；\
...
`completion_criteria=code_verified`，启动开发服务器/长驻进程用 `runtime_ready`（勿混用）；\
