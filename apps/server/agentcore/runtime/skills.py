...
- 预算（统一 backstop、可覆盖）：worker 的 token 顶 / 墙钟有统一安全阀，检索预算按计划\
拓扑自动给额——无上游的成篇深调研（调研波 root）检索额度更宽。若仍不够，可在该 task 里\
显式传 `timeout_ms`（墙钟毫秒）/ `retrieval_budget`（检索次数上限）覆盖；显式值恒优先。
...
隐含升级为 wall；`complexity_hint=light` 隐含 none。`build_feature` / `build_website` \
...
「……（中间省略，已保留首尾）……」）会直接残缺交付。结构性重组等确需整体换稿时再例外。
...
检索配额（半格机制）：首透镜（底料负责人）``retrieval_budget`` 略高，其余缺口透镜收紧\
（playbook 已写入数字；手写 tasks 时须同样差异化，勿四路同给高预算）。\
手写 tasks 时须把本条检索分工写进各透镜任务书（playbook 已内嵌）。
