...
async def test_takeover_unknown_conversation_is_404(client):
...
async def test_takeover_non_owner_is_404(client, new_client):
...
async def test_start_without_session_reports_no_session(client):
    await register_and_login(client, "btuser4")
...
async def test_input_conflict_when_not_active(client):
...
async def test_end_is_idempotent_when_not_active(client):
    await register_and_login(client, "btuser6")
...
async def test_takeovers_list_starts_empty(client):
    await register_and_login(client, "btuser7")
