...
/**
 * IM 图片消息：缩略图网格 + lightbox（打开 / 关闭 / 下载 / 失败态）。
 * mock fetchChatAttachmentBlob / share；桩 createObjectURL；Modal 简化为 dialog 壳。
 */

import type { StoredAttachment } from "@/api/messaging";
import {
  cleanup,
  fireEvent,
  render,
...
vi.mock("@/components/Modal", () => ({
  Modal: ({
    ...
    // biome-ignore lint/a11y/useSemanticElements: Modal mock for tests — mirrors lightweight dialog shell.
...
import { fetchChatAttachmentBlob } from "@/api/messaging";
...
describe("ChatImageGallery", () => {
  ...
  it("downloads the original from the lightbox toolbar", async () => {
    ...
    await waitFor(() => {
      expect(mockDownload).toHaveBeenCalledWith(expect.any(Blob), "photo.png");
    });
  });
